...
Most often, running backs in the National Football League are compared by taking summary statistics from each play. Traditional metrics include yards-per-carry, expected points added per play (EPA), and win probability added per play (WPA). While comparing backs based on average yards, EPA, and WPA is the most straightforward approach to compare players, averages can be particularly sensitive to outlying observations and distributions with heavy tails. Unfortunately, each of the metrics above are all strongly skewed right (EPA also has outliers to the left), which means that taking averages may not be an optimal approach.


As part of the NFL's Big Data Bowl, participants dove deeper into running back performance. In this post, I'll use one set of Big Data Bowl predictions (provided by the Zoo, [link](https://www.kaggle.com/c/nfl-big-data-bowl-2020/discussion/119400)) to examine two newer metrics -- yards over expectation and yardage percentiles. 
...
Here's a histogram of yards over expectation. 
...
plot_yoe <- df_yoe %>% 
  ggplot(aes(yoe)) + 
  geom_histogram() + 
  labs(title = "Distribution of yards over average") + 
  theme_bw(12)
library(patchwork)
plot_yds + plot_yoe
```

Yards above expectation is centered at 0 and is slightly more symmetric, but there remain outliers and a heavy tail to the right. That is, despite the Zoo's projections [being accurate on aggregate](https://operations.nfl.com/stats-central/stats-articles/what-the-top-predictions-looked-like-in-the-nfls-big-data-bowl/), yards over expectation boasts a fairly similar distribution to yards.

The skewness of each distribution, combined with the relatively small number of carries that are afforded to running backs each season, can make it difficult to infer much about player talent using averages alone. Damien Williams' 91 yard touchdown run, for example, was only expected to gain 5.2 yards. In the histograms above, there's not much difference between 91 yards and 95.8 yards above expectation. 
...
Big Data Bowl predictions aren't just expected yards -- they're distributions. Here's the empirical cumulative density function (CDF) for two sample carries, a Cordarrelle Patterson two-yard loss and a Raheem Mostert 41-yard touchdown run.
...
As an alternative comparison, an 8-yard gain for Patterson is roughly the equivalent of what Mostert did on his touchdown run. 

Side benefits of using these CDFs? 

First, they're intuitive -- the cdf is a percentile. Patterson losing 2 yards and Mostert (hypothetically) gaining 8 yards both correspond to the 30th percentile.

Second, percentiles will (roughly) follow a uniform distribution, which means they aren't as prone to outliers.
...
Ezekial Elliot's typical carry ended in the 67th-percentile of what a typical running back would gain, while Devonta Freeman was in the 37th-percentile. Roughly, it was just as common for Freeman to finish in the top-quarter of running back distributions as it was for Elliot to finish in the top half. Le'Veon Bell, Chris Carson, and Jordan Howard are other names atop the chart.
...
Freeman, Elliott, and Williams are all highlighted, with performances that appear consistent in the two samples. Williams, with his long touchdown run reduced from extreme outlier to simply the 100th-percentile, is no longer as impacted by an outlier.

The correlation in the above plot is 0.40 -- that matches what occurs if you take a few thousand such random samples.

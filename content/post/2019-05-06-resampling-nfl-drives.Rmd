...
As part of the NFL's offseason, the league's competition committee and owners debated the merits of a rule change, proposed by [Kansas City](https://operations.nfl.com/updates/football-ops/2019-playing-rules-proposals/), that would allow both teams the opportunity to possess the ball at least one time in overtime. In the current rulebook, if the team possessing the ball first scores a touchdown on its opening drive, the game ends. 

There were a few ways in which the league used statistics to supplement offseason conversations regarding overtime. Given that the Chiefs' amendment was tabled until 2020, it's also likely a topic that will be revisted. But in the meantime, a funky question came up on our analytics side --  

**How do you estimate NFL outcomes under a set of rules that don't exist?**

Kansas City's proposed format could create a scenario unlike any other in football. If the game starts with an opening drive touchdown, the receiving team would: 

(i) Needs a touchdown to extend the game
(ii) Avoid punts altogether
(iii) Avoid field goals altogether
(iv) Not particularly care about the clock

Points (ii) and (iii) are key. It is not uncommon for teams to need a touchdown when trailing late, but in those situations, a number of other factors also play a role in decision making, including timeouts, the likelihood of getting a defensive stop after a possible punt or field goal attempt, and number of timeouts. 
...
In absence of building several such models, a second, simpler approach would use the observed set of plays and some resampling. That is, for each play, we'll sample from the emperical distribution of similar plays to estimate everything that would happen on that play. 

Here's a version of what this looks. Given that our internal data is a slightly more enhanced than what's out there publicly, for this post I modified our code to accomodate the public play-by-play data from [nflscrapr](https://github.com/maksimhorowitz/nflscrapR). Each of these plays are stored in a data frame `df.plays`, grabbed using code [here](https://github.com/statsbylopez/BlogPosts/blob/master/scrapr-data.R). 

### Pre-processing

First, to ensure team motivations roughly fall in line with those participating in an overtime game, I'll strip the entire set of NFL plays into scrimmage plays run in one possession games, outside of the last two minutes of each half. I also drop two-point conversions, create indicator for whether a fumble or interception occured (`is.turnover`), whether an offensive (`is.td.offense`) or defensive (`is.td.defense`) touchdown occured, and add a variable for line-of-scrimmage (yards from own goal, or `yfog`). 
...
The offense started with a Andy Dalton pass and ended with a Dez Bryant touchdown. In between -- and in what shows the importance of replicating what would happen in overtime -- the offense went for it on 4th-2 in its own territory. That play, a Mike Gillislee run, is shown in red.
...
The above simulation dodges a few things that complicate football drives, most notably penalties. In our work at the league office, we got a number closer to 40% than 30%. That is, when you add in penalties, which tend to help the offense more than they help the defense, you'll likely get a rate higher than the 30.2% I got above. 

For now, hopefully this gives you a sense of how resampling can help better understand circumstances in games that don't exist. In our case, 